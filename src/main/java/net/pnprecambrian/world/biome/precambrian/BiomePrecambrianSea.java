...
public class BiomePrecambrianSea extends ElementsPNPrecambrianMod.ModElement {
	@GameRegistry.ObjectHolder("lepidodendron:precambrian_sea")
	public static final BiomeGenCustom biome = null;
	public BiomePrecambrianSea(ElementsPNPrecambrianMod instance) {
		super(instance, 1591);
	}
	...
	static class BiomeGenCustom extends BiomePrecambrian {
		public BiomeGenCustom() {
			super(new BiomeProperties("Ediacaran Shallow Sea").setRainfall(0.95F).setBaseHeight(1.7F).setHeightVariation(0.07F).setTemperature(1.1F));
	    ...
	    public void decorate(World worldIn, Random rand, BlockPos pos)
	    {
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS))
				for (int i = 0; i < 12; ++i)
				{
					...
					int k = rand.nextInt(16) + 8;
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					STATIC_GENERATOR.generate(BlockTawuia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 1, 255, 65, 255);
				}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS)) {
				...
				for (int i = 0; i < Math.ceil(8D * edicarandensity); ++i) {
					...
					if (worldIn.getBiome(pos.add(j, l, k)) == this) {
						STATIC_ROTATIONAL_GENERATOR.generate(BlockCharnia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 8, 255, 0, 255);
					}
				}
			}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS)) {
				...
				for (int i = 0; i < Math.ceil(8D * edicarandensity); ++i) {
					...
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					if (worldIn.getBiome(pos.add(j, l, k)) == this) {
						STATIC_ROTATIONAL_GENERATOR.generate(BlockParacharnia.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 8, 255, 60, 255);
					}
				}
			}
			...
			if(net.minecraftforge.event.terraingen.TerrainGen.decorate(worldIn, rand, new net.minecraft.util.math.ChunkPos(pos), net.minecraftforge.event.terraingen.DecorateBiomeEvent.Decorate.EventType.GRASS)) {
				for (int i = 0; i < 16; ++i) {
					...
					int l = rand.nextInt(worldIn.getHeight(pos.add(j, 0, k)).getY() + 32);
					if (worldIn.getBiome(pos.add(j, l, k)) == this) {
						STATIC_GENERATOR.generate(BlockFinkoella.block.getDefaultState(), worldIn, rand, pos.add(j, l, k), 3, 60, 65, 255);
					}
				}
			}
